...
const { data, status, clear, execute, error } = useFetch(`/api/posts/${post.id}`, {
  method: 'PATCH',
  body: formData,
  watch: false,
  immediate: false,
...
const handleUpdate = async ({ valid }) => {
  ...
  if (valid) {
    ...
    if (status.value === 'error') {
      ...
      // if (data.value) post.value.push(data.value)
    }
